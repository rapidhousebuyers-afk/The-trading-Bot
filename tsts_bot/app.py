...
- MULTI-SETUP DETECTION: MULTIPLE trade setups can exist SIMULTANEOUSLY on the same timeframe. A LOGO (read via V-shape + sniper divergence) and a GGG RGG (read via dir bars/BS/Bokk bottom-up alignment) can both be present on the same 5m panel at the same moment. They are different readings of the same price action. Classify each independently.
