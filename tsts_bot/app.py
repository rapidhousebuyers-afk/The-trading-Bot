...
- MULTI-SETUP DETECTION: A single chart can contain MULTIPLE setups of DIFFERENT TYPES, especially on the 5m. Example: a LOGO (V-shape with sniper divergence) AND a GGG RGG (bottom-up build with dir bars/BS/Bokk all aligning green). Scan the ENTIRE chart history, not just the rightmost candle. Each setup must be classified by its own trade type — do NOT assume all setups on one chart are the same type.
